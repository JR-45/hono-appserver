...
```shell
    # Windows:
    $env:DATABASE_URL='postgresql://mitglied:p@localhost/mitglied?schema=mitglied&connection_limit=10&sslnegotiation=direct?sslcert=src/config/resources/postgresql/server.crt'

    # macOS:
    DATABASE_URL='postgresql://mitglied:p@localhost/mitglied?schema=mitglied&connection_limit=10&sslnegotiation=direct?sslcert=src/config/resources/postgresql/server.crt'
```
...
- PascalCase für die Model-Namen, z.B. `B`uch statt `b`uch.
  - Bei jedem umbenannten Model muss am Ende `@@map` ergänzt werden, z.B. @@map("mitglied").
- camelCase für die Field-Namen, z.B. `mitgliedId` statt `mitglied_id`.
  - Bei jedem umbenannten Field muss `@map` ergänzt werden, z.B. @map("mitglied_id").
...
Außerdem sollte in `prisma\schema.prisma` der eigene Schema-Name (hier: `mitglied`)
eingetragen werden:
...
try {
  ...
  console.log(`mitglieder=${JSON.stringify(mitglieder)}`);
} finally {
  await prisma.$disconnect();
}
```

Der obige Prisma-Client `prisma.mitglied` greift auf die Tabelle `mitglied` in der
...
Schema `mitglied` auswählen muss, um die Tabellen anzuzeigen:

```shell
    # Windows:
    $env:DATABASE_URL='postgresql://mitglied:p@localhost/mitglied?schema=mitglied'

    # macOS / Linux:
    # DATABASE_URL='postgresql://mitglied:p@localhost/mitglied'
    DATABASE_URL='postgresql://mitglied:p@localhost/mitglied?schema=mitglied'
