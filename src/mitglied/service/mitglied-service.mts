...
export class MitgliedService {
    ...
    async findById({
        ...
        const include = mitAusleihen
            ? this.#includeAusweisUndAusleihen
            : this.#includeAusweis;
        const mitglied: MitgliedMitAusweisUndAusleihen | null =
            await prismaClient.mitglied.findUnique({
                where: { id },
                include,
            });
        if (mitglied === null) {
     ...
     */
    async findFileByMitgliedId(
        mitgliedId: number,
    ): Promise<Readonly<MitgliedFile> | undefined> {
        this.#logger.debug('findFileByMitgliedId: mitgliedId=%d', mitgliedId);
        const mitgliedFile: MitgliedFile | null =
            await prismaClient.mitgliedFile.findUnique({ where: { mitgliedId } });
   ...
   /**
    ...
    async #findAll(
        pageable: Pageable,
    ): Promise<Readonly<Slice<MitgliedMitAusweis>>> {
        const { number, size } = pageable;
        const mitglieder: MitgliedMitAusweis[] = await prismaClient.mitglied.findMany({
            skip: number * size,
            take: size,
    ...
    #checkEnums(suchparameter: Suchparameter) {
        ...
        const validMitgliedsstatus =
            mitgliedsstatus === undefined ||
            mitgliedsstatus === 'AKTIV' ||
            mitgliedsstatus === 'INAKTIV';
